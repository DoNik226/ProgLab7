...
public class Loader<T extends Collection<E>, E> {
    ...
    public Loader(Class<T> tClass, Class<E> eClass)
    {
        ...
        {
            System.out.println("В программе возникла проблема: " + e);
        }
    }
    ...
    public T loadFromXMLbyEnvKey(String envKey) {
        String xmlPath = System.getenv(envKey) ; //"C:/Users/nikdo/IdeaProjects/laba5/labworks.xml";
        if (xmlPath == null) {
            System.out.println("Отсутствует переменная окружения с путем к загрузочному файлу!");
            System.out.println("Задайте переменную окружения с именем \"laba5\", поместив туда полный путь к XML файлу.");
            System.out.println("Выполнение программы не может быть продолжено.");
            System.exit(-1);
        }
    ...
    public T loadFromFile(String path, BaseReader reader)
    {
        try {
            ...
            System.out.println("/ ! \\ File reading failed because application cannot access to this file. Contact your administrator and ask him to write \'sudo chmod 755 $labа6\'");
            ...
            System.out.println("Во время работы с вводом-выводом произошла непредвиденная ошибка! " + e.getMessage());
        } catch (Exception e) {
    ...
    private void addFieldToElement(String[] pathToField, String s) {
        ...
        if (matcher.find())
        {
            ...
            System.out.println("Поле имеет некорректный адрес и будет пропущено. " + Arrays.toString(pathToField));
            return;
        }
        ...
        for (int i = 2; i < pathToField.length; i++)
        {
            try {
                ...
                System.out.println("Поле не было обнаружено: " + e);
            }
        }
    }
    ...
    private <U> void setField (Class<?> currentType, String[] fullPath, int currentIndex, String value, U object) throws NoSuchFieldException, InstantiationException, IllegalAccessException {
        ...
        else
        {
            ...
            {
                System.out.println("XML файл поврежден. Строка с данными " + value + " (Адрес: " + Arrays.toString(fullPath) + ") была пропущена.");
                System.out.println("Если вы считаете, что произошла ошибка, добавьте в регистр обработчик. Взгляните на Loader.setupConverter в Javadoc или свяжитесь с администратором.");
            }
        }
    }
    ...
    private E buildElement() {
        ...
        try
        {
            ...
            System.out.println("При инициализации элемента возникла проблема! " + e.getMessage());
